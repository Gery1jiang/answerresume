...
export default function ApplicantProfileModal({ open, onClose }: Props) {
  ...
  const handleSelectPoi = useCallback((poi: { name: string; lng: number; lat: number }) => {
    form.setFieldsValue({ home_address: poi.name, home_lng: poi.lng, home_lat: poi.lat });
        ...
        {loading ? (
          ...
          <Form form={form} layout="horizontal" labelCol={{ span: 6 }} wrapperCol={{ span: 14 }}>
            <Form.Item label="家庭住址" name="home_address">
              <Input.Search placeholder="请输入或选择地址" enterButton="选择地址" onSearch={() => handleOpenMapModal()} />
            </Form.Item>
      ...
      >
        <div>
          <Input.Search
            placeholder="搜索地址..."
            value={searchKeyword}
            onChange={(e) => { setSearchKeyword(e.target.value); if (e.target.value.trim()) handleSearch(e.target.value); }}
          ...
          <div style={{ position: 'relative' }}>
            ...
            {!searchLoading && searchResults.length > 0 && (
              <div style={{ maxHeight: 480, overflow: 'auto', border: '1px solid var(--admin-border)', borderRadius: 4, marginBottom: 8, background: 'var(--admin-bg, #0f1626)' }}>
                {searchResults.map((poi, idx) => {
                  const [lng, lat] = poi.location.split(',').map(Number);
                  return (
                    <div key={`${poi.location}-${idx}`}
                      onClick={() => { setSelectedPoi({ name: poi.name, lng, lat }); flyToPoi(lat, lng, poi.name); }}
                      style={{ padding: '8px 12px', cursor: 'pointer', borderBottom: '1px solid var(--admin-border)', color: 'var(--admin-text)' }}
