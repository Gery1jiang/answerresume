...
_KEYWORD_ROUTES: list[tuple[re.Pattern, str]] = [
    (re.compile(r"(?:帮(?:我)?)?(?:抓取|爬取|搜索|搜|抓|找).*?(?:招聘|岗位|职位|工作)"), "kimi_crawl_tool"),
    (re.compile(r"(?:帮(?:我)?)?(?:匹配|评分|评估|分析).*(?:匹配|适合|评估|评分|度|性)"), "match_jobs_tool"),
    (re.compile(r"(?:帮(?:我)?)?(?:生成|做|制作|写).*简历"), "generate_resume_tool"),
    (re.compile(r"(?:帮(?:我)?)?(?:再?)做(?:一个|一份|个|一次)"), "generate_resume_tool"),
    (re.compile(r"(?:帮(?:我)?)?(?:解析|读取|提取|打开|读).*(?:文件|图片|文档|PDF|附件|内容|文字)"), "parse_file_tool"),
    (re.compile(r"(?:帮(?:我)?)?(?:新增|创建|添加|加).*面试"), "create_interview_record_tool"),
    (re.compile(r"(?:帮(?:我)?)?(?:生成|创建|制作|做).*面试.*报告"), "generate_interview_report_tool"),
    (re.compile(r"(?:帮(?:我)?)?(?:取消|停止|终止|停).*报告"), "cancel_report_tool"),
    (re.compile(r"(?:查|看|查询|检查|搜).*报告.*(?:进度|状态|情况)"), "check_report_status_tool"),
    (re.compile(r"(?:搜索|搜|查|找).*(?:新闻|信息|资料|网站|网页)"), "web_search_tool"),
    (re.compile(r"(?:帮(?:我)?)?(?:修改|更新|编辑|换|改).*(?:知识|简历|经历|信息)"), "knowledge_preview"),
    (re.compile(r"(?:重建|刷新|重新).*(?:索引|向量|知识库)"), "knowledge_rebuild_vector"),
    (re.compile(r"(?:帮(?:我)?)?(?:识别|提取).*(?:图片|截图|照片|图像)"), "parse_file_tool"),
    (re.compile(r"(?:查|查询|查看|搜索|搜).*(?:会话|聊天|对话|记录)"), "query_sessions_tool"),
    (re.compile(r"(?:查|查询|查看|搜索|搜).*(?:统计|数据|系统|指标)"), "query_system_data_tool"),
]
